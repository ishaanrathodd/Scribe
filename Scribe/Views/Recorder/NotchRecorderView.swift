...
struct NotchRecorderView<S: RecorderStateProvider & ObservableObject>: View {
    ...
    private var displayState: DisplayState {
        ...
        case .recording:
            let shouldShowLive = showLiveTranscript && !isAskMode
            return shouldShowLive ? .liveText : .active
        case .enhancing:
            let shouldShowLive = showLiveTranscript && !isAskMode && !stateProvider.partialTranscript.isEmpty
            return shouldShowLive ? .liveText : .active
        case .transcribing:
            return .active
        default:
    ...
    private let transcriptPanelHeight: CGFloat = 78
    ...
    private var liveTextPanel: some View {
        VStack(spacing: 0) {
            if displayState == .liveText {
                ...
                LiveTranscriptView(text: stateProvider.partialTranscript, lineLimit: 2)
